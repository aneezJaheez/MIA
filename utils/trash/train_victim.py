...
def train_step(model, train_loader, criterion, optimizer, epoch, device, log_interval=10):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            if len(targets.shape) >= 2:  # show mse only when the labels are one-hot or soft
                ...
                print(
                    '[Train] Epoch: {:.2f} [{}/{} ({:.0f}%)]\tCE Loss: {:.6f}\tAccuracy: {:.1f} ({}/{})'.format(
                        exact_epoch, batch_idx * len(inputs), len(train_loader.dataset),
                        100. * batch_idx / len(train_loader),
...
def test_step(model, test_loader, criterion, device, epoch=0., silent=False):
    ...
    if not silent:
        ...
        print('[Test]  Epoch: {}\tCE Loss: {:.6f}\tAcc: {:.1f}% ({}/{})'.format(epoch, test_loss, acc,
                                                                                correct, total))
...
def train_model(model, trainset, out_path, batch_size=64, criterion_train=None, criterion_test=None, testset=None,
                device=None, num_workers=10, lr_step=30, lr_gamma=1e-7,
